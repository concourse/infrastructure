...
locals {
  concourse_stress_values = templatefile("${path.module}/concourse-values.yml.tpl",
    {
      cluster_name = "stress"
...
locals {
  stress_workers_values = templatefile("${path.module}/concourse-worker-values.yml.tpl",
    {
      cluster_name = "stress"
