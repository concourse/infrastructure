...
resource "google_service_account" "dispatcher_vault" {
  account_id   = "dispatcher-vault"
  display_name = "Production Vault"
  description  = "Used to operate Vault in our Production cluster."
}

resource "google_storage_bucket" "dispatcher_vault" {
  name = "concourse-dispatcher-vault"
...
resource "google_project_iam_member" "dispatcher_vault_policy" {
  for_each = {
  ...
  role = each.value
  member = "serviceAccount:${google_service_account.dispatcher_vault.email}"
}

resource "google_service_account_iam_binding" "dispatcher_vault_workload_identity" {
  service_account_id = google_service_account.dispatcher_vault.name
...
resource "google_storage_bucket_iam_member" "dispatcher_vault_policy" {
  bucket = google_storage_bucket.dispatcher_vault.name
